...
using UnityEngine;
    ...
    public class ExampleNetworkedEntityView : ColyseusNetworkedEntityView
    {
		...
		public int seat; //MODH
		private bool firstSeatCorrection = true; //NMODH
		public int eSeat; //NMODH
		public bool theGameStarted = false; //MODH
		public int tTries = 0; //MODH
        ...
        virtual protected void UpdateViewFromState()
        {
			...
			//myTransform.localPosition = Vector3.Lerp(myTransform.localPosition, pos, Time.deltaTime * (positionLerpSpeed + 0.2f)); //NMODH WEBGL

			eSeat = (int)state.eSeat; //NMODH
			seat = (int)state.seat;
			theGameStarted = state.gameStarted;
			tTries = (int)state.tries;
			theScore = (int)state.score;
        ...
        protected virtual void UpdateStateFromView()
        {
			...
			eSeat = (int)state.eSeat; //NMODH
			if(firstSeatCorrection && eSeat != 0) //NMODH
				{
					GameObject.Find("gameSceneManager").GetComponent<GameSceneManager>().playerNumber = (uint)eSeat; //NMODH
					firstSeatCorrection = false; //NMODH
				}
			state.seat = GameObject.Find("gameSceneManager").GetComponent<GameSceneManager>().playerNumber;
			state.gameStarted = GameObject.Find("gameSceneManager").GetComponent<GameSceneManager>().itStarted;
			state.tries = this.gameObject.GetComponent<ShootAI>().tries;
			state.score = this.gameObject.GetComponent<UiManager>().myScore;
        ...
        {
			/////////////////  NMODH NMODH  WEBGL//////////////
            if (this.gameObject.name == "Basket Ball") {
			IsMine = true;
        }
		else
        {
		...
		/////////////////  NMODH NMODH  WEBGL//////////////
		syncLocalPosition = true; //NMODH WEBGL
		syncLocalRotation = true; //NMODH WEBGL
        ...
        protected virtual void ProcessViewSync()
        {
            ...
            if (/*proxyStates[0].timestamp > interpolationTime*/true) //GAMBIARRA NMODH para consertar o bug no WEBGL
            {
			...
			if (syncLocalPosition)
			{
				...
				if (state.zVel > 0.5f)
					deltaFactor = 0.2f;
				//myTransform.localPosition = new Vector3(myTransform.localPosition.x, (proxyStates[0].pos.y * 0.55f + myTransform.localPosition.y * 0.55f + 1.123f), myTransform.localPosition.z);
				else
					myTransform.localPosition = new Vector3(myTransform.localPosition.x, proxyStates[0].pos.y + 1.123f, myTransform.localPosition.z);
			}
